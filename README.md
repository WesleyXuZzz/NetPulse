...
```bash
git clone https://github.com/WesleyXuZzz/net-pulse.git
cd net-pulse
./scripts/build-app.sh
```

打包完成后，打开 `dist/net-pulse.dmg`，将 `net-pulse.app` 拖拽到 Applications。安装后，macOS 中显示的应用名称为“网速监控”。

`./scripts/build-app.sh` 默认使用 release 构建，并会重新生成 `dist/net-pulse.app` 和 `dist/net-pulse.dmg`；如果这些路径下已有旧的本地打包产物，会被新的产物覆盖。
...
你可以打开 `net-pulse.dmg`，将 `net-pulse.app` 拖拽到 Applications 后再使用。
...
如果你是通过 `swift run` 直接运行源码，macOS 不会把它当成正式的 `.app` 安装包来处理。请先执行 `./scripts/build-app.sh`，再从打包后的 `net-pulse.app` 中开启“登录自启”；安装后系统中显示的应用名称为“网速监控”。
